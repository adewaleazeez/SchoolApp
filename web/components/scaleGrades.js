class ScaleGrades extends React.Component {
    constructor(props) {
        super(props);

        this.state = {
            scaleGrades: [], updating: false, loading: false, updateView: false, newRecord: false
        }
    ...
    componentDidMount() {
        $('#breadcrumb').html(`<li><a href="#/"><i class="fa fa-home"></i> Home</a></li><li>Setup</li><li class="active">Scales and Grades</li>`);
    ...
    async loadData() {
        ...
        await APICall("scaleGrades.json", {}, "GET")
            .then(data => {
                console.log(data);
                if (data.status == "success") {
                    this.setState({ scaleGrades: data.data, loading: false });
                }
            })
            .catch(error => {
                console.error(error);
...

                        {
                            !this.state.updateView ?
                                ...
                                <div style={{maxWidth: "800px"}} className="panel panel-primary animated rotateIn faster">
                                    <div className="panel-heading">
                                        <div className="panel-title">
                                            <h5 style={{ marginTop: "30px" }}><strong>Scales and Grades Setup</strong></h5>
                                    ...
                                    <div className="panel-body">
                                        ...
                                        <table className="table table-striped" style={{ "fontSize": "90%" }}>
                                            <thead>
                                                <tr><th></th><th>Scale</th><th>Grade</th></tr>
                                            </thead>
                                            <tbody>
                                                {
                                                    [].concat(this.state.scaleGrades)
                                                        ...
                                                        .map((item, i) =>
                                                            <tr key={item.id} className="clickable">
                                                                ...
                                                                <td onClick={() => this.showUpdate(item.id)} style={{ width: "200px" }}><strong className="text-info">{item.scale}</strong></td>
                                                                <td onClick={() => this.showUpdate(item.id)} >{item.grade}</td>
                                                            </tr>
                                                        )
                                ...
                                <div style={{ maxWidth: "800px" }} className="panel panel-primary animated rotateIn faster">
                                    <div className="panel-heading">
                                        <div className="panel-title">
                                            <h5 style={{ marginTop: "30px", marginLeft: "-10px" }}><strong>New Scale/Grade</strong></h5>
                                        </div>
                                    </div>
                                    <div className="panel-body">
                                        <form >
