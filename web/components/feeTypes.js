class FeeTypes extends React.Component {
    constructor(props) {
        ...
        this.state = {
            feeTypes: [], updating: false, loading: false, updateView: false, newRecord: false
        }
    ...
    componentDidMount() {
        $('#breadcrumb').html(`<li><a href="#/"><i class="fa fa-home"></i> Home</a></li><li>Setup</li><li class="active">Fee Types</li>`);
    ...
    async loadData() {
        ...
        await APICall("feeTypes.json", {}, "GET")
            .then(data => {
                console.log(data);
                if (data.status == "success") {
                    this.setState({ feeTypes: data.data, loading: false });
                }
            })
            .catch(error => {
                console.error(error);
...

                        {
                            !this.state.updateView ?
                                ...
                                <div style={{maxWidth: "800px"}} className="panel panel-primary animated rotateIn faster">
                                    <div className="panel-heading">
                                        <div className="panel-title">
                                            <h5 style={{ marginTop: "30px" }}><strong>Fee Types Setup</strong></h5>
                                            <button style={{ marginTop: "15px", marginBottom: "30px", marginLeft: "0px" }}
                                                ...
                                                <i className="fa fa-plus"></i> Add Fee Type</button>
                                        </div>
                                    </div>
                                    <div className="panel-body">
                                        ...
                                        <table className="table table-striped" style={{ "fontSize": "90%" }}>
                                            <thead>
                                                <tr><th></th><th>Fee Type</th><th>Bank</th></tr>
                                            </thead>
                                            <tbody>
                                                {
                                                    [].concat(this.state.feeTypes)
                                                        ...
                                                        .map((item, i) =>
                                                            <tr key={item.id} className="clickable">
                                                                ...
                                                                <td onClick={() => this.showUpdate(item.id)} >{item.bank}</td>
                                                            </tr>
                                                        )
                                ...
                                <div style={{ maxWidth: "800px" }} className="panel panel-primary animated rotateIn faster">
                                    <div className="panel-heading">
                                        <div className="panel-title">
                                            <h5 style={{ marginTop: "30px", marginLeft: "-10px" }}><strong>New Fee Type</strong></h5>
                                        </div>
                                    </div>
                                    <div className="panel-body">
                                        <form >
