class MenuRole extends React.Component {
    ...
    switchCheckBox(itemId, status){
        APICall("http://localhost:5000/menurole/swith-checkbox", {id: itemId, status: status}, "PUT")
        .then(data => {
            if (data.status === 200) {
    ...
    loadRoleData(){
        ...
        APICall("http://localhost:5000/usertypes/selectbyorgid", {organizationId: sessionStorage.getItem("orgid")}, "POST")
        .then(data => {
    ...
    loadTopMenuData(){
        //this.setState({ loading: true });
        APICall("http://localhost:5000/topmenu/selectbyorgid", {organizationId: sessionStorage.getItem("orgid")}, "POST")
        .then(data => {
            if (data.status === 200) {
    ...
    loadSubMenuData(id){
        //this.setState({ loading: true });
        APICall("http://localhost:5000/submenu/selectbytopmenu", {parentId: id, organizationId: sessionStorage.getItem("orgid")}, "POST")
        .then(data => {
            if (data.status === 200) {
    ...
    populateDataTable = async () => {
        this.setState({ loading: false });
        const response = await APICall("http://localhost:5000/menurole/selectbyroleid", 
                ...
                { "data": "id",
                    render: function(data, type, row, meta) {
                        if (type === "display") {
                            ...
                            var delete_button = "<button class='to-delete btn btn-danger btn-sm' data-id='" + row.id + "' data-topMenu='" + row.topMenu + "' data-subMenu='" + row.subMenu + "' data-url='" + row.url + "' data-status='" + row.status + "' ><i class='fa fa-trash text-danger' style='cursor: pointer; color: white;' title='Delete'></i>&nbsp;Delete</button>";
    ...
    saveRecord() {
        ...
        APICall("http://localhost:5000/menurole/save", 
        {  
    ...
    updateRecord() {
        ...
        //this.setState({ updating: true });
        APICall("http://localhost:5000/menurole/update", 
        {
    ...
    showUpdate(itemId) {
        //this.setState({ loading: true });
        APICall("http://localhost:5000/menurole/selectbyid", {id: itemId}, "POST")
        .then(data => {
            if (data.status === 200) {
    ...
    startDelete(itemId, usertype, topMenu, subMenu, url, status_delete) {
        swal({
            ...
            if (isConfirm) {
                APICall("http://localhost:5000/menurole/delete", 
                {
                    id: itemId
                }, 
                "DELETE").then(data => {
                    ...
                    <div className=" border-3-top flatpanel">
                        {
                            ...
                            (
                                ...
                                <div style={{ maxWidth: "800px" }} className="panel panel-primary animated rotateIn faster">
                                    <div className="panel-heading">
                                        <div className="panel-title">
                                            <h5 style={{ marginTop: "30px", marginLeft: "-10px" }}><strong>Add User Type Menus</strong></h5>
                                        </div>
                                    </div>
                                                ...
                                                <div className="col-md-6 col-sm-12 col-lg-6">
                                                    <div className="row">
                                                            ...
                                                            <div className="form-group">
                                                                ...
                                                                <select className="select2 js-states form-control custom_select" id="submenu_add" ref="submenu_add">
                                                                    <option value='0'>Select a sub menu...</option>
                                                                    {
                                                                        ...
                                                                        .sort((a, b) => a.id > b.id)
                                                                        .map((item, i) =>
                                                                            <option key={item.id} url={item.url} className="clickable" value={item.id}>{item.name}</option>
                                                                        )
                                                                    }
                                                                </select>
                                                            </div>
                                                        ...
                                                        <div className="col-md-12">
                                                            <div className="form-group">
                                                                <label>Url</label>
                                                                <input type="text" className="form-control" disabled defaultValue="" ref="url_add" />
                                                            </div>
                                                        </div>
                                                        <div className="col-md-12">
                                                            <div className="form-group">
                                                                <label>Status</label>
                                                                <input id="status_add" ref="status_add" type="checkbox" disabled className="switch-checkboxes" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="col-md-6 col-sm-12 col-lg-6">
                                                <div className="row">
                                                    <div className="col-md-12">
                                                        <br/>
                                                        <label>Select User Type</label>
                                                        <select className="select2 js-states form-control custom_select" id="usertypes_update">
                                                            <option value='0'>Select a User Type...</option>
                                                            {
                                                                [].concat(this.state.roleData)
                                                                .sort((a, b) => a.id > b.id)
                                                    ...
                                                    <div className="col-md-12">
                                                        <div className="form-group">
                                                            ...
                                                            <input type="hidden" className="form-control" defaultValue={this.state.menuRoleData.id} ref="id_update" />
                                                            <input type="hidden" className="form-control" defaultValue={this.state.menuRoleData.organizationId} ref="organizationId_update" />
                                                            <input type="hidden" className="form-control" defaultValue={this.state.menuRoleData.menuId} ref="parentId_update" />
                                                            <input type="text" className="form-control" defaultValue={this.state.menuRoleData.name} ref="name_update" />
                                                        </div>
                                                    </div>
                                                    <div className="col-md-12">
                                                        <div className="form-group">
                                                            <label>Icon</label>
                                                            <input type="text" className="form-control" defaultValue={this.state.menuRoleData.icon} ref="icon_update" />
                                                        </div>
                                                    </div>
                                                    <div className="col-md-12">
                                                        <div className="form-group">
                                                            <label>Url</label>
                                                            <input type="text" className="form-control" defaultValue={this.state.menuRoleData.url} ref="url_update" />
                                                        </div>
                                                    </div>
                                                    <div className="col-md-12">
                                                        <div className="form-group">
                                                            <label>Position</label>
                                                            <input type="text" className="form-control" defaultValue={this.state.roleData.rank} ref="rank_update" />
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
